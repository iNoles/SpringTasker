...
class JwtUtils {
    ...
    private fun getSigningKey(): Key {
        return SecretKeySpec(jwtSecret.toByteArray(), SignatureAlgorithm.HS512.jcaName)
    }

    fun getJwtFromCookies(request: HttpServletRequest) : String? {
        return WebUtils.getCookie(request, jwtCookie)?.let {
            it.value ?: null
        }
    }
    ...
    fun clearJwtCookie(): ResponseCookie {
        return ResponseCookie.from(jwtCookie, "") // Set value to an empty string
